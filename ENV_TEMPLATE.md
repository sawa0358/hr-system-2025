...
# Cron自動実行設定
# =====================================
# Cronエンドポイント認証トークン（自動実行用、外部cronサービス使用時のみ必要）
# 生成コマンド: openssl rand -base64 32
...
# 本番環境では自動的に有効化されます
# 開発環境でテストする場合は 'true' に設定
AUTO_START_VACATION_SCHEDULER=false

# =====================================
# カード期限通知自動実行設定
# =====================================
# カード期限通知の自動実行スケジューラーを有効化（コード内で自動実行）
# 本番環境では自動的に有効化されます
# 開発環境でテストする場合は 'true' に設定
AUTO_START_CARD_REMINDER_SCHEDULER=false
```
