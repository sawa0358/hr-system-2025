# バックアップ復元手順 (v2.9.1以前に戻す)

**作成日時**: 2024年11月28日 21:42:39  
**バージョン**: v2.9.0 → v2.9.1 移行前のバックアップ  
**変更内容**: WorkClockWorkerテーブルに `transferDestination` (振込先) フィールドを追加

---

## 📦 バックアップ内容

このディレクトリには以下のファイルが含まれています：

1. **dev.db.backup** - SQLiteデータベースファイル（バイナリ）
2. **dev.db.sql** - SQLiteデータベースのSQLダンプ（テキスト形式）
3. **schema.prisma.backup** - Prismaスキーマファイル
4. **prisma_folder_backup/** - prismaフォルダ全体のコピー

---

## 🔄 復元方法

### 方法1: データベースファイルを直接復元（推奨・最速）

```bash
# 1. 現在のデータベースをバックアップ（念のため）
cp ./prisma/prisma/dev.db ./prisma/prisma/dev.db.before-restore

# 2. バックアップから復元
cp backups/pre_v2.9.1_20251128_214239/dev.db.backup ./prisma/prisma/dev.db

# 3. スキーマファイルも復元
cp backups/pre_v2.9.1_20251128_214239/schema.prisma.backup prisma/schema.prisma

# 4. Prisma Clientを再生成
npx prisma generate

# 5. サーバーを再起動
npm run dev
```

---

### 方法2: SQLダンプから復元（完全復元）

```bash
# 1. 既存のデータベースを削除
rm ./prisma/prisma/dev.db

# 2. SQLダンプから復元
sqlite3 ./prisma/prisma/dev.db < backups/pre_v2.9.1_20251128_214239/dev.db.sql

# 3. スキーマファイルも復元
cp backups/pre_v2.9.1_20251128_214239/schema.prisma.backup prisma/schema.prisma

# 4. Prisma Clientを再生成
npx prisma generate

# 5. サーバーを再起動
npm run dev
```

---

### 方法3: prismaフォルダ全体を復元（最も安全）

```bash
# 1. 現在のprismaフォルダをバックアップ
mv prisma prisma.before-restore

# 2. バックアップから復元
cp -r backups/pre_v2.9.1_20251128_214239/prisma_folder_backup prisma

# 3. Prisma Clientを再生成
npx prisma generate

# 4. サーバーを再起動
npm run dev
```

---

## ⚠️ 注意事項

1. **復元前に必ず現在の状態をバックアップしてください**
2. 復元後は必ず `npx prisma generate` を実行してください
3. サーバーを完全に停止してから復元作業を行ってください
4. 本番環境（Heroku等）の復元は、このバックアップではなく本番環境のバックアップを使用してください

---

## 🔍 バックアップの検証

バックアップが正常に作成されているか確認：

```bash
# データベースファイルのサイズを確認
ls -lh backups/pre_v2.9.1_20251128_214239/dev.db.backup

# SQLダンプの内容を確認
head -20 backups/pre_v2.9.1_20251128_214239/dev.db.sql

# スキーマファイルの内容を確認
grep "transferDestination" backups/pre_v2.9.1_20251128_214239/schema.prisma.backup
# → 見つからなければ正常（v2.9.0のスキーマ）
```

---

## 📞 問題が発生した場合

1. サーバーを完全に停止
2. `.next` フォルダを削除: `rm -rf .next`
3. `node_modules/.prisma` を削除: `rm -rf node_modules/.prisma`
4. 上記の復元手順を再実行
5. それでも解決しない場合は、このファイルと一緒に問い合わせてください

---

**このバックアップは安全に保管してください。**

